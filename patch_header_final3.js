...
const regex = /<button[\s\S]*?className="rounded-2xl bg-slate-900 px-5 py-3 text-sm font-semibold text-white transition hover:bg-slate-800"[\s\S]*?>[\s\S]*?\{t\.export\}[\s\S]*?<\/button>/;

const replacement = `<div className="flex items-center gap-2 z-50">
                  {!connected ? (
                    <button
                      type="button"
