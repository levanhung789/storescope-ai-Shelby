...
const regex = /<button[\s\S]*?className="rounded-2xl bg-slate-900 px-5 py-3 text-sm font-semibold text-white transition hover:bg-slate-800"[\s\S]*?>\s*\{t\.export\}\s*<\/button>/g;
...
if(matches.length > 0) {
  ...
  text = text.replace(lastMatch, replacement);
