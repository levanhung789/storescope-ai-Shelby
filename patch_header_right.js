...
const topLeftPattern = `              <div className="flex flex-col gap-1">
...
const restoreOverview = `              <div>
                <p className="text-sm font-semibold uppercase tracking-[0.2em] text-cyan-600">
                  {t.overview}`;

text = text.replace(topLeftPattern, restoreOverview);

// Add to top-right (before export button)
const exportButtonPattern = `<button
                  type="button"
...
const topRightWalletButton = `<div className="flex items-center gap-2">
                  {!connected ? (
                    <button
                      type="button"
                      onClick={() => connect("Petra" as any)}
                    ...
                    <button
                      type="button"
                      onClick={() => disconnect()}
                      className="rounded-2xl border border-cyan-600 bg-cyan-50 px-5 py-3 text-sm font-semibold text-cyan-800 transition hover:bg-cyan-100 flex items-center gap-2 group relative"
                    >
                      <span className="w-2 h-2 rounded-full bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.8)] animate-pulse"></span>
                      {account?.address?.toString().slice(0, 4)}...{account?.address?.toString().slice(-4)}
                      <span className="absolute -bottom-10 left-1/2 -translate-x-1/2 rounded bg-slate-800 px-2 py-1 text-[10px] text-white opacity-0 transition group-hover:opacity-100 pointer-events-none whitespace-nowrap">Thoat lien k?t vi</span>
                    </button>
                  )}
                </div>
                <button
...
text = text.replace(exportButtonPattern, topRightWalletButton);

fs.writeFileSync('app/dashboard/client.tsx', text);
console.log('Header patched to top-right');
